...
var _mockjs = __webpack_require__(336);
...
//兼容性
var syy = new _lottery2.default(); //引入lottery.js模块
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(93), __webpack_require__(90)(module)))
...
function _classCallCheck(instance, Constructor) { if (!(instance instanceof Constructor)) { throw new TypeError("Cannot call a class as a function"); } } //整合彩票业务模块的4个业务，相当于一个完整的项目模块
//引入各模块
...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //用来记录彩票项目的一些可以多次复用的文字数据
    ...
    _createClass(Base, [{
        ...
        * */
        ...
        value: function toggleCodeActive(e) {
            var self = this;
            var $cur = (0, _jquery2.default)(e.currentTarget); //获取当前被选中的DOM;
        ...
        value: function changePlayNav(e) {
            var self = this;
            var scur = (0, _jquery2.default)(e.currentTarget); //返回的是子元素
        ...
        value: function assistHandle(e) {
            ...
            var index = $cur.index();
            (0, _jquery2.default)('.boll-list .btn-bol').removeClass('btn-bol-active');
            if (index === 0) {
                (0, _jquery2.default)('.boll-list .btn-bol').addClass('btn-bol-active');
            }
            if (index === 1) {
        ...
        value: function addCode() {
            var self = this;
            var $active = (0, _jquery2.default)('.boll-list .btn-bol-active').text().match(/\d{2}/g);
        ...
        value: function addCodeItem(code, type, typeName, count) {
            ...
            (0, _jquery2.default)(self.cart_el).append(tpl); //将购物车号码添加上去
    ...
    _createClass(Timer, [{
        ...
        value: function countdown(end, update, handle) {
            ...
            if (now - end > 0) {
                ...
                var px_d = 1000 * 60 * 60 * 24; //记录一天是多少毫秒（1天24小时，1小时60分钟，1分钟60秒，1秒是1000毫秒）
                var px_h = 1000 * 60 * 60; //1天的时间和1小时的时间差24倍（表示小时）
                var px_m = 1000 * 60; //分钟和小时的关系是60倍（表示分）
                var px_s = 1000;
                var d = Math.floor(last_time / px_d); //看看剩余时间包含多少天
                var h = Math.floor((last_time - d * px_d) / px_h); //剩余多少小时
                var m = Math.floor((last_time - d * px_d * px_h) / px_m); //剩余的分钟数
                var s = Math.floor((last_time - d * px_d * px_h - m * px_m) / px_s); //剩余秒数
                var r = []; //值保存值结果中
                if (d > 0) {
...
var Calculate = function () {
    ...
    _createClass(Calculate, [{
        ...
        value: function computeCount(active, play_name) {
            ...
            * */
            var count = 0; //声明默认注数
            var exist = this.play_list.has(play_name); //判断玩法列表中是否存在play_name
            //生成数组
            var arr = new Array(active).fill('0'); //es6数组初始化的时候允许传入一个参数就是数组的长度
            //上面是一个指定长度为被选中的号码的个数为长度的数组，各元素默认均为0
            if (exist && play_name.at(0) === 'r') {
                //当前玩法是否支持，并且判断当前玩法是否是r
        ...
        value: function computeBonus(active, play_name) {
            var play = play_name.split(''); //获取玩法的分类
            var self = this;
        ...
        value: function combine(arr, size) {
            ...
            (function f(arr, size, result) {
                //es6中的函数必须有名字，在该处命名为f，es6在递归的时候要用匿名函数，否则报错
                var arrLen = arr.length; //判断传入数组长度
                if (size > arrLen) {
                    //长度小于size,
                    return; //截止递归
                }
                if (size === arrLen) {
                    ...
                    for (var i = 0; i < arrLen; i++) {
                        var newResult = [].concat(result); //不断生成新数组的过程
...
var _createClass = function () { function defineProperties(target, props) { for (var i = 0; i < props.length; i++) { var descriptor = props[i]; descriptor.enumerable = descriptor.enumerable || false; descriptor.configurable = true; if ("value" in descriptor) descriptor.writable = true; Object.defineProperty(target, descriptor.key, descriptor); } } return function (Constructor, protoProps, staticProps) { if (protoProps) defineProperties(Constructor.prototype, protoProps); if (staticProps) defineProperties(Constructor, staticProps); return Constructor; }; }(); //很多数据需要从后台的接口获取，需要建立该模块用来处理接口数据，接口模块
